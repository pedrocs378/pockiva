...
- Pull requests targeting `main` must come from `develop` in `pedrocs378/pockiva`. Their only required workflow check is `Validate release candidate`, which compares metadata from isolated base and candidate checkouts using the trusted script pinned to `github.workflow_sha`, the exact default-branch revision whose `pull_request_target` workflow GitHub is executing.
...
- Before creating or reusing the pull request, the trusted script from the workflow revision calculates the exact patch tree. The gate accepts only changes to `Cargo.lock`, `Cargo.toml`, `apps/desktop/package.json`, and `apps/desktop/src-tauri/tauri.conf.json`. A stale branch, unexpected tree, missing pull request, duplicate pull request, or other inconsistent remote state fails closed without force-pushing.

The first `develop -> main` release carries the explicit `0.1.1` bump from PED-84. GitHub loads `pull_request_target` workflows from the repository default branch (`develop`), so trusted executable tooling is checked out at `github.workflow_sha`; the `main` base and `develop` candidate are checked out separately as data. After that merge, require only `Validate release candidate` on `main`; keep the three task CI contexts required on `develop`.
...
If preparation or either build fails, keep the GitHub release as a draft and inspect the failed job. A transient or external failure can be retried for the same merged commit; if the workflow definition itself was faulty, ship its correction through the next patch release because rerunning an old attempt does not load a newer workflow revision. Drafts are discovered through the release listing because GitHub's public tag endpoint does not return unpublished releases. Preparation accepts exactly one existing draft only when its tag and `target_commitish` match the merged commit; a published version, a conflicting tag, duplicate matches, or a draft pointing elsewhere fails closed. Asset verification and publication use the immutable release ID. Immediately before publication, the workflow creates or validates the lightweight tag at the exact release commit, then confirms that the published tag still resolves to that commit.
